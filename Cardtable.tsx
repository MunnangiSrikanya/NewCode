...
const CardComponent: React.FC = () => {
  ...
  const headers = [
    ...
    { key: 'pxCreateOpName', label: 'Updated By' },
      ...
      <Grid item xs={12} className='header-row'>
        <Grid container className='card-header'>
          {headers.map((header, index) => (
            <Grid item xs={2} key={index} className='grid-layout'>
              ...
              <Grid container>
                {headers.map(
                  (header, headerIndex) =>
                    header.key !== 'pyStatusWork' && (
                      <Grid item xs={2} key={headerIndex}>
                ...
                <Grid item xs={2}>
                  <Button
                    variant='contained'
                    className={
                      row.pyStatusWork === 'Submitted' ? 'submitted-button' : row.pyStatusWork === 'In Progress' ? 'in-progress-button' : 'new-button'
                    }
                  >
                    {row.pyStatusWork}
                  </Button>
